...
# Path definitions
data_root = '/home/lucijatonkovic/Documents/Data/CIL/'
predictions_dir = os.path.join(data_root, 'PredictionDepths/NoTuning/DepthAnythingV2')
test_list_file = os.path.join(data_root, 'ethz-cil-monocular-depth-estimation-2025/test_list.txt')
output_csv = os.path.join(data_root, 'Prediction_CSVs/predictions_dav2_notune.csv')
...
def process_depth_maps(with_inversion=False):
    # Read file list
    ...
    for rgb_path, depth_path in tqdm(file_pairs, desc="Processing depth maps"):
        ...
        # Invert values
        if with_inversion:
            val_to_add = (-1) * (depth * (-1)).min()
            depth = (depth * (-1)) + val_to_add # to ensure values are non-negative
...
if __name__ == "__main__":
    process_depth_maps(with_inversion=True) 